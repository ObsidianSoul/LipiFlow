 ...
 * Must be .jsx (not .js) because renderInline() returns JSX elements.
 */
...
/**
 * Converts inline markdown to styled JSX:
 *   **text** → <strong>
 *   `text`   → <code>
...
export function renderInline(text = '', key = '') {
    ...
    return parts.map((part, i) => {
        if (part.startsWith('**') && part.endsWith('**')) {
            return (
                <strong key={`${key}-b${i}`} className="text-white font-semibold">
                    {part.slice(2, -2)}
                </strong>
            );
        }
        if (part.startsWith('`') && part.endsWith('`')) {
            return (
                <code key={`${key}-c${i}`} className="text-violet-300 bg-violet-500/10 px-1.5 py-0.5 rounded text-sm font-mono">
                    {part.slice(1, -1)}
                </code>
            );
        }
